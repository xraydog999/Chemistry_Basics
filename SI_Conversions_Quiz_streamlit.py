...
units = ["m"]  # You can expand later (g, L, etc.)
...
if "score" not in st.session_state:
    ...
    st.session_state.total_questions = 8  # number of questions per quiz
...
if st.session_state.quiz_over:
    ...
    for r in st.session_state.results:
        if r["correct"]:
            ...
            st.write(f"❌ {r['question']} → You answered: {r['user']} (Correct: {r['correct_answer']})")
...
def generate_question():
    ...
    # Avoid trivial same-prefix conversions
...
# Store question if not already stored
